...
| Manual | `/admin/manual` (also `/admin/manual/ghl-custom-fields`, `/admin/manual/roadmap`) | This manual and its companion docs rendered in-app from `docs/*.md`, with an on-page table of contents. Opened in a new tab from the **?** beside the theme switch (desktop top bar and the mobile drawer footer) |
...
| Date | Change |
| --- | --- |
| 2026-09-15 | **Manual in the app**: `/admin/manual` renders this manual (plus `ghl-custom-fields.md` and `roadmap.md`) from the repo's `docs/` folder with `marked`, heading anchors, an "On this page" list, and doc-to-doc links rewritten to in-app routes (`src/lib/admin/manual.ts`). A **?** icon beside the theme switch opens it in a new tab. `outputFileTracingIncludes` ships the Markdown with the Vercel function. |
