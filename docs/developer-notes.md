...
| Client replies | GHL → app | GHL workflow (Customer Replied → Webhook, §5) posts `ghl_contact_id` to `POST /api/ghl/replies` → `ghl_contact_replies.last_inbound_at`. Opening the contact's conversations drawer (or replying from it) stamps `seen_at`, clearing the flag |
...
| Signed-step retries | app → GHL / storage | Signed contracts with steps left in `signed_actions_pending` (or never started) are retried on their own: event page load (after the response), the Contracts page sweep and Refresh statuses, 2+ minutes apart, at most 10 automatic runs (`signed_actions_attempts`); the webhook and Refresh status always retry |
...
- **Signed side effects** (`applySignedContractActions`): four independent
  ...
  `follow_ups` (pause lifted), `signed_pdf` (executed PDF archived). A run is
  claimed first (the first run stamps `signed_actions_applied_at` where it is
  null; retries claim on `updated_at`), so the webhook, the signer and a
  page load can't double-apply. Steps that fail go into
  `signed_actions_pending` and only those re-run later
  (`retryPendingSignedContracts`) — a finished step never repeats, so rooms a
  coordinator changes after signing aren't re-booked. A "skipped" outcome
...
| `SUPABASE_STORAGE_BUCKET` | Where signed contract PDFs are archived (defaults to `event-uploads`). Must name a bucket that exists: in September 2026 archives failed with `Bucket not found (bucket "PORTAL_BASE_URL")`, i.e. the variable held another variable's name. Failed archives now retry on their own once it's fixed |
