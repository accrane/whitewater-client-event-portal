...
| Conversations drawer snippet menu | GHL → app | Location snippets (`/locations/{id}/templates`), read live and cached 5 min per server process (never stored); merge tags rendered per contact/coordinator (`/api/ghl/contacts/[contactId]/message-templates`) |
